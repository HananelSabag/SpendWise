 ...
 * Full-page wishlist with category filtering, price totals, add/edit BottomSheet
 */
...
// ──────────────────────────────────────────
const formatTotal = (n) =>
  '₪' + (parseFloat(n) || 0).toLocaleString('he-IL', { minimumFractionDigits: 0, maximumFractionDigits: 2 });
...
const ShoppingWishlistPage = () => {
  ...
  const {
    items, total, isLoading, isError, refetch,
  ...
  const categoryTabs = useMemo(() => {
    ...
    return ['הכל', ...CATEGORIES.filter((c) => usedCats.includes(c.value)).map((c) => c.value)];
  }, [items]);
  ...
  // Split bought / not bought
  const [unbought, bought] = useMemo(() => {
    const u = filteredItems.filter((i) => !i.is_bought);
    const b = filteredItems.filter((i) => i.is_bought);
    return [u, b];
  }, [filteredItems]);

  const pendingTotal = useMemo(
    () => unbought.reduce((s, i) => s + parseFloat(i.price_ils || 0), 0),
    [unbought]
  );
                ...
                >
                  ...
                  {active && (
                    <span className="bg-white/30 text-white text-[10px] font-extrabold px-1.5 rounded-full">
                      {cat === 'הכל' ? items.length : items.filter((i) => i.category === cat).length}
                    </span>
                  )}
                </motion.button>
              );
