...
describe('cycle optimistic cache patches', () => {
  it('removes the last recurring rule immediately without retaining an obsolete fallback', () => {
    expect(
      applyRecurringGroupPatch(
        { recurringGroups: [{ id: 'only-rule' }] },
        { groupId: 'only-rule', active: false },
      ).recurringGroups,
    ).toEqual([]);
  });
  it('changes estimate and engine settings without waiting for a refetch', () => {
    ...
    expect(
      applyCycleSettingsPatch(data, { engineMode: 'manual', manualAnchorDay: 15 }).settings,
    ).toEqual({
      engineMode: 'manual',
      manualAnchorDay: 15,
      useEstimates: true,
  ...
  it('patches both the current and history cache shapes for a card', () => {
    ...
    const result = applyCardSettingsPatch(
      { cycle: { cards: [card] }, cycles: [{ cards: [card] }] },
      {
        source: 'max',
        accountNumber: '2254',
        included: false,
    ...
    expect(result.cycle.cards[0]).toMatchObject({
      included: false,
      statementDay: { day: 15, certain: true, source: 'user' },
    });
  ...
  it('renames and toggles a recurring rule immediately', () => {
    ...
    expect(result.recurringGroups[0]).toMatchObject({
      label: 'Mortgage',
