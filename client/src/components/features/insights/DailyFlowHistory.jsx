...
function compactMoney(value) {
  return new Intl.NumberFormat('he-IL', { notation: 'compact', maximumFractionDigits: 1 }).format(Number(value) || 0);
}
...
function FlowTooltip({ active, payload, label, he, formatCurrency }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white/95 p-3 text-xs shadow-xl backdrop-blur dark:border-gray-700 dark:bg-gray-900/95">
      <p className="mb-2 font-bold text-gray-900 dark:text-white">{formatDate(label, he ? 'he' : 'en')}</p>
      <p className="text-rose-600">{he ? 'יצא היום' : 'Out today'}: {formatCurrency(values.spentCommitted || 0)}</p>
      <p className="text-emerald-600">{he ? 'נכנס היום' : 'In today'}: {formatCurrency(values.incomeExSalary || 0)}</p>
      <p className="mt-1 text-indigo-600">{he ? 'נטו מצטבר' : 'Cumulative net'}: {formatCurrency(values.cumulativeNetExSalary || 0)}</p>
    </div>
  );
}
    ...
    <section className="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
      <div className="border-b border-gray-100 p-4 dark:border-gray-800 sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-indigo-500">
              <Clock3 className="h-4 w-4" />{he ? 'משכורת למשכורת' : 'Salary to salary'}
            </p>
            <h2 className="mt-1 text-lg font-black text-gray-950 dark:text-white">{he ? 'מה קרה בכל יום במחזור' : 'What happened each day'}</h2>
            <p className="mt-1 text-xs text-gray-500">{he ? 'עובדות בלבד — בלי תחזית ובלי לספור חיוב אשראי פעמיים.' : 'Facts only — no forecast and no double-counted card settlement.'}</p>
          </div>
          ...
          <div className="inline-flex rounded-2xl bg-gray-100 p-1 dark:bg-gray-800">
            {['current', 'previous'].map((key) => (
              ...
              >
                {key === 'current' ? (he ? 'נוכחי' : 'Current') : (he ? 'קודם' : 'Previous')}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="mt-4 flex items-center gap-2 text-xs text-gray-500">
          ...
          <span>·</span>
          <span>{cycle?.daysElapsed || 0} {he ? 'ימים' : 'days'}</span>
        </div>
      </div>

      <div className="p-4 sm:p-5">
        <div className="grid grid-cols-3 gap-2 sm:gap-3">
          <Metric icon={TrendingDown} label={he ? 'יצא' : 'Out'} value={cycle?.money?.spentCommitted} tone="red" formatCurrency={formatCurrency} />
          <Metric icon={TrendingUp} label={he ? 'נכנס בלי שכר' : 'In, ex salary'} value={cycle?.money?.incomeExSalary} tone="green" formatCurrency={formatCurrency} />
          <Metric icon={Wallet} label={he ? 'נטו במחזור' : 'Cycle net'} value={cycle?.money?.netExSalaryCommitted} tone="indigo" formatCurrency={formatCurrency} />
        </div>
        ...
        <div className="mt-5 h-64 w-full" dir="ltr">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={history} margin={{ top: 8, right: 4, bottom: 0, left: 0 }}>
              ...
              <YAxis tickFormatter={compactMoney} tick={{ fontSize: 10, fill: '#9ca3af' }} axisLine={false} tickLine={false} width={42} />
              <Tooltip content={<FlowTooltip he={he} formatCurrency={formatCurrency} />} />
        ...
        <div className="mt-4 flex flex-wrap gap-3 text-[11px] text-gray-500">
          <span className="flex items-center gap-1"><i className="h-2.5 w-2.5 rounded-sm bg-rose-400" />{he ? 'יצא ביום' : 'Daily out'}</span>
          <span className="flex items-center gap-1"><i className="h-2.5 w-2.5 rounded-sm bg-emerald-400" />{he ? 'נכנס בלי משכורת' : 'Income excluding salary'}</span>
          <span className="flex items-center gap-1"><i className="h-0.5 w-3 bg-indigo-500" />{he ? 'נטו מצטבר' : 'Cumulative net'}</span>
        </div>

        <div className="mt-5 border-t border-gray-100 pt-4 dark:border-gray-800">
          <h3 className="mb-2 text-sm font-bold text-gray-900 dark:text-white">{he ? 'יומן יומי' : 'Daily ledger'}</h3>
            ...
            {activeDays.map((day) => (
              <div key={day.date} className="flex items-center gap-3 py-3">
                <div className="w-16 shrink-0">
                  ...
                  <p className="text-[10px] text-gray-400">{day.transactionCount} {he ? 'תנועות' : 'items'}</p>
                </div>
                <div className="min-w-0 flex-1">
                  {day.salaryIncome > 0 && <span className="inline-flex rounded-full bg-indigo-50 px-2 py-0.5 text-[10px] font-bold text-indigo-600 dark:bg-indigo-950/30">{he ? 'נכנסה משכורת' : 'Salary received'} · {formatCurrency(day.salaryIncome)}</span>}
                  {day.spentPending > 0 && <p className="mt-1 text-[10px] text-amber-600">{formatCurrency(day.spentPending)} {he ? 'עדיין ממתין' : 'still pending'}</p>}
                  {day.needsReviewCount > 0 && <p className="mt-1 text-[10px] text-amber-600">{day.needsReviewCount} {he ? 'דורש בדיקה' : 'needs review'}</p>}
                </div>
                <div className="text-end text-xs tabular-nums">
                  ...
                  {day.spentCommitted < 0 && <p className="font-bold text-emerald-600">+{formatCurrency(Math.abs(day.spentCommitted))} {he ? 'החזר' : 'refund'}</p>}
                </div>
                {he ? <ChevronLeft className="h-4 w-4 text-gray-300" /> : <ChevronRight className="h-4 w-4 text-gray-300" />}
              </div>
            ))}
            {!activeDays.length && <p className="py-6 text-center text-sm text-gray-500">{he ? 'אין פעילות במחזור הזה.' : 'No activity in this cycle.'}</p>}
          </div>
        </div>
      </div>
    </section>
