...
**Current cycle schema note (2026-07-22):** migrations 28–35 are applied to production. V6 automatic windows use the latest included card statement day, count every salary received inside that billing window, and keep salary tracking as an income signal rather than an anchor. Estimate mode controls only conservative card growth. Migration 35 adds named recurring groups, per-group forecast inclusion, per-card statement-day/include/link settings, and calculation version 6.
