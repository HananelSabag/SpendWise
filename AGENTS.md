...
> Last synced to live DB + code: 2026-09-12. Owner: Hananel (super_admin, user id=1).
...
**The shared grocery list is no longer part of this app.** It ran here until September 2026 as a
second app inside the first — own routes, own bottom nav, own first-run picker — and moved out to
`C:\CodingProjects\grocery` (GitHub `HananelSabag/grocery`), where it has no server at all: the
browser talks to Supabase directly and RLS carries the authorization. Migration 45 dropped its
tables here. If you find a reference to it anywhere in this repo, that reference is stale.
...
- Google login requires the user's session. Dev-only cycle/family preview harnesses provide synthetic UI verification without credentials; they do not replace authenticated end-to-end checks.
...
- **Family Hub (migration 43 — manual, not derived from bank data):** `family_monthly_items(kind income|fixed|variable|loan|savings, name, amount ₪/MONTH, owner_user_id NULLable = joint, category_key, charge_day, is_active, notes, + loan-only lender/outstanding_amount/payments_left/end_date, created_by/updated_by)` and `family_balances(kind savings|pension|study_fund|investment|emergency|other, name, institution, owner_user_id NULLable, amount ₪ RIGHT NOW, monthly_contribution, as_of, is_active)`. **Neither table is scoped per user** — they hold ONE household's shared picture and access is the allowlist in `config/familyAccess.js`. RLS enabled with no policies (server is owner). See §11.
- `notifications: id, user_id, type, title, body, data(jsonb), is_read` — `type` + `data` are the contract; `title`/`body` are an English **fallback only** and the client renders text from its own translations (`components/layout/notificationPresentation.js`). Never store display text that a user of the other language will have to read.
...
- **routes/**: `transactionRoutes, bankConnectionsRoutes, bankSyncRoutes, bankAgentRoutes(/bank-agent/* — agent job claim/complete), agentPairingRoutes, authStatusRoutes, userRoutes, adminRoutes, familyRoutes(/family/* — household overview + manual monthly items and balances), notificationRoutes, exportRoutes, healthRoutes, onboarding`.
- **controllers/**: `transactionController`(slim, ~385L — aggregation moved to services), `familyController`, `notificationController`, `exportController`, `auth/`, `admin/`.
- **services/** (business logic lives here): `bankConnectionHealth`(derived sync health), `bankSyncService`(ingest: dedup, tombstones, kind-gates balance), `syncSchedulingService`(enqueue-on-claim), `dashboardService`(dashboard aggregation), `monthlyAccountingService` + `calendarMonthSummaryService` + `calendarActivityService`(calendar-month rollups), `financialClassificationService`(ledger_class), `salaryReviewService`(salary signatures), `cardReconciliationService`(bank↔card settlement match), `bankPendingDedupService`(pending→settled), `merchantWatchService`, `familyBudgetService`(household arithmetic, pure) + `familyTaxonomy`(its key vocabulary), `cycleRunwayService`(projection/runway), `dashboardClassifierWidgetsService`, `accountScopeService` + `agentClaimScope`(scoping), `exchangeRateService`, `exportDataService`, `emailService`, `supabaseStorage`(profile pictures), `authStatusService`.
- **models/**: `Transaction, User, UserCache, Notification, FamilyBudget` (thin data-access).
- **middleware/**: `auth`(strict — logout/optional handled globally; refresh no longer falls back to JWT_SECRET), `rateLimiter`, `security`, `validate`, `errorHandler`, `maintenance`, `requestId`, `routeLogger`, `upload`, `familyAccess`(**the only place Family Hub authorization is decided**).
...
- entry `main.jsx` / `App.jsx`; API layer `api/` (`client.js` axios core + per-domain: `transactions, bankConnections, agentPairing, auth, authStatus, admin, notifications, onboarding, family`).
...
- **pages/**: `ModernDashboard`, `ModernTransactions`, `FinancialCyclePageV2`(/financial-cycle — lazy-exported as `InsightsPage`, which is why the route and the file disagree), `YearlyReviewPage`(/financial-cycle/yearly/:year), `BankSyncPageV2`, `Profile`, `FamilyHubPage`(/family), `QuickExpensePage`, `admin/`, `auth/`, plus `Blocked/Maintenance/ServerWaking/NotFound`.
- **components/features/**: `admin, auth, bankSync, dashboard, exchange, family, financialCycleV2, help, insights, profile, transactions`. Shared UI in `components/ui/` (incl. `LiquidTabs.jsx` — the animated segmented tabs used app-wide; `fill` mode = no horizontal scroll for user pages, scroll allowed for Admin).
- **hooks/**: `useDashboard, useTransactions, useTransactionActions, useFinancialCycle, useFinancialPeriodSelection, useFinancialDataSync, useCalendarMonthSummary, useCalendarMonthDetails, useBankSyncMonitor, useNotifications`(persistent notification centre, `hooks/useNotifications.js`) vs `useAppNotifications`(transient toasts, re-exported from `stores/` — do NOT confuse them), `useExport, useOnboarding*, useIsMobile, useToast…`.
...
- **`FINANCIAL_CYCLE_SPEC.md`** — THE normalized cycle-engine spec (processedDate model, card classification, bank↔card reconciliation, v8 billing-day cycle, links, projection). **Validated to the agora against real bank data (fixture in §7).** Read before touching any cycle/accounting/reconciliation code. This supersedes scattered logic in the older handoff docs.
...
## 11. FAMILY HUB  (migration 43 — `/family`, two accounts only)

The one screen in the app whose data is typed in by hand. Everything else is derived from what the
banks already did, which is accurate about the past and silent about the question a couple actually
asks before the month starts: **of what comes in, how much is already spoken for, and how much is
left for us to live on?** Answering it needs facts no scraper reports — which salary is whose, which
standing charge leaves whose account, how much is in a pension, how much is still owed on a loan.

- **It is a plan, not a ledger.** No row here touches `transactions`, the cycle engine, or any
  accounting total, and nothing is ever created here from bank data. Keep it that way: the moment
  the two are mixed, the same shekel gets counted twice.
- **Two tables, two meanings of "amount"** (§4): `family_monthly_items.amount` is ₪ per MONTH,
  `family_balances.amount` is ₪ RIGHT NOW. Splitting them is what stops a ₪120,000 pension from
  being subtracted from a month.
- **A loan is deliberately both**, in two different places: its monthly repayment is an outflow this
  month, its `outstanding_amount` is a debt against the household's assets. Those totals never mix.
  A loan with no balance entered counts monthly and adds nothing to debt — an incomplete debt figure
  beats an invented one, and `debt.loansMissingBalance` says so.
- **`monthly_contribution` on a balance is never subtracted from anything.** A pension or study-fund
  deposit comes out of GROSS pay; charging it against a net salary invents an expense that never
  happens. It is displayed as context. A transfer they really do make each month belongs in
  `family_monthly_items` as `kind='savings'` instead. Pinned by a test.
- **The headline is narrow on purpose:** `available = income − (fixed + loans + savings)`. Variable
  spending is NOT committed — it is what the leftover is *for*, and folding it in would hide the one
  number the screen exists to show. It appears separately as `variablePlanned` → `projected`.
- **`owner_user_id NULL` means joint** — a real answer, not a missing one. Per-person figures are how
  "how much leaves MY account vs hers" gets answered when charges are split across two banks. A
  shared bucket has no income, so its negative is expected and must never render as an alarm.
- **Authorization is ONE allowlist check** in `middleware/familyAccess.js`, reading
  `config/familyAccess.js`. The tables have no per-row ownership; everyone on the list shares one
  dataset. `created_by`/`updated_by` are history, never permission. Do not add a second rule anywhere.
- **The allowlist is account IDs, never email addresses.** This repo is public and the members are a
  private household. `FAMILY_HUB_USER_IDS` / `FAMILY_HUB_EMAILS` override it from the environment
  (Render env or a local `.env`) — never from a tracked file. A test asserts no address is hard-coded.
- **The client never carries the allowlist**: `normalizeUserData` puts a server-computed `familyHub`
  boolean on the user, and the nav entries (Header, MobileBottomNav) key off that. It decides what is
  *offered*; the server is the gate.
- **Every mutation answers with the whole recomputed overview**, and `useFamilyBudget` writes that
  straight into the cache. The totals are the point of the screen — a phone and a laptop typing into
  it together must not drift. There is no optimistic total and no polling (a refetch on focus covers
  the other person's edits on this sit-down-together screen).
- **Suggestion chips fill a name and a category, never an amount.** A screen whose whole value is an
  honest picture must not pre-fill a number nobody checked.
- Dev harness: `client/family-preview.html` + `src/dev/FamilyPreview.jsx` mount the three panels with
  synthetic data and the REAL translations, so RTL/dark/mobile are reviewable without credentials.
  Synthetic only — never leave captured household data in a tracked file.
