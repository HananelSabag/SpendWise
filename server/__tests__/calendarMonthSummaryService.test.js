...
describe('calendar month card reconciliation', () => {
  test('counts connected card purchases by purchase date and fully excludes their bank settlement and reversal', () => {
    const rows = [
    ...
    expect(result.totals).toMatchObject({
      ...
      creditCardDebitAdjustments: 12000,
      bankReversalAdjustments: 12000,
    });
    ...
    expect(result.reconciliation.adjustments).toEqual(expect.arrayContaining([
      expect.objectContaining({ cardSource: 'max', bankDebit: 12000, alreadyRepresented: 3000, adjustment: 12000, remainingBankDebit: 0 }),
  ...
  test('does not exclude an unrelated same-amount bank income from another account', () => {
    ...
    const result = buildCalendarMonthSummaryFromRows(rows, accounts, rows, period);
    expect(result.totals).toMatchObject({ income: 12000, expenses: 0, net: 12000 });
